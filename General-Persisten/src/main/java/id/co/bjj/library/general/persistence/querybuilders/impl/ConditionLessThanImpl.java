...
@Component("conditionLessThan")
public class ConditionLessThanImpl<M> implements WherePredicateBuilder<M>, ConditionComparer<M> {
	@Override
	...
	public Predicate getPredicateString(CriteriaBuilder criteriaBuilder, Root<M> root, ParameterDto parameter) {
		try {
			...
			if (parameter.getSeparator().equals(SeparatorEnum.AND)) {
				return criteriaBuilder.and(criteriaBuilder.lessThan(root.get(parameter.getFieldName()), firstValue));
			} else if (parameter.getSeparator().equals(SeparatorEnum.OR)) {
				return criteriaBuilder.or(criteriaBuilder.lessThan(root.get(parameter.getFieldName()), firstValue));
			} else {
