...
@Component("conditionLessThanEquals")
public class ConditionLessThanEqualsImpl<M> implements WherePredicateBuilder<M>, ConditionComparer<M> {
	@Override
		...
		try {
			...
			if (parameter.getSeparator().equals(SeparatorEnum.AND)) {
				return criteriaBuilder
						.and(criteriaBuilder.lessThanOrEqualTo(root.get(parameter.getFieldName()), firstValue));
			} else if (parameter.getSeparator().equals(SeparatorEnum.OR)) {
				return criteriaBuilder
						.or(criteriaBuilder.lessThanOrEqualTo(root.get(parameter.getFieldName()), firstValue));
			} else {
