...
@Component("conditionNotEquals")
public class ConditionNotEqualsImpl<M> implements WherePredicateBuilder<M> {
	@Override
	public Predicate getPredicate(CriteriaBuilder criteriaBuilder, Root<M> root, ParameterDto parameter) {
		if (parameter.getSeparator().equals(SeparatorEnum.AND)) {
			return criteriaBuilder
					.and(criteriaBuilder.notEqual(root.get(parameter.getFieldName()), parameter.getFirstValue()));
		} else if (parameter.getSeparator().equals(SeparatorEnum.OR)) {
			return criteriaBuilder
					.or(criteriaBuilder.notEqual(root.get(parameter.getFieldName()), parameter.getFirstValue()));
		} else {
