...
@Component("conditionGreaterThanEquals")
public class ConditionGreaterThanEqualsImpl<M> implements WherePredicateBuilder<M>, ConditionComparer<M> {
	@Override
		...
		try {
			...
			if (parameter.getSeparator().equals(SeparatorEnum.AND)) {
				return criteriaBuilder
						.and(criteriaBuilder.greaterThanOrEqualTo(root.get(parameter.getFieldName()), firstValue));
			} else if (parameter.getSeparator().equals(SeparatorEnum.OR)) {
				return criteriaBuilder
						.or(criteriaBuilder.greaterThanOrEqualTo(root.get(parameter.getFieldName()), firstValue));
			} else {
